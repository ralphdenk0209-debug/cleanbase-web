...
(function(){
  ...
  const KNOTEN = [
    ...
    {id:"ein_front",c:3, r:0, f:Y, t:"Eingang 3: Frontbild ohne EAN",
      s:"App 'Produkt ohne Barcode fotografieren' → cb_riki_ohne_ean_einreihen(Fotos) → legt sofort ein Entwurfsprodukt 'RIKI-Entwurf ohne EAN' an (Identitaet = Foto-Fingerabdruck in riki_no_ean_identity) → riki_scan_job. RIKI liest Name, Marke, Kategorie und - seit v27 - den Barcode, wenn er im Bild ist (Worker v5 nimmt ihn nur mit gueltiger Pruefziffer; eine falsch gelesene Ziffer repariert ean-reparatur-takt alle 5 Min nur mit Gegenbeleg von Open Food Facts: gleiche Marke und Namenswort). Der Auftrag endet planmaessig in 'gehalten'; der Wortlaut kommt aus der Beschaffung, nicht vom Foto."},
    ...
    {id:"offd",  c:3, r:6,  f:Y, t:"Open Food Facts: Name, Naehrwerte, Zutaten", s:"off-rohtext-takt (alle 6 Min) und off-namen-takt (alle 3 Min) holen Wortlaut und Namen zur EAN → Zutaten_Rohtext Quellenart Open Food Facts. off-naehrwerte-takt (alle 10 Min; cb_off_naehrwerte_takt → Edge off-lookup) holt die Naehrwerte je 100 g, nur wenn Open Food Facts sie vollstaendig und stimmig hat. Ohne Ballaststoffe bleibt die Naehrwertachse offen.", p:"Produkte mit EAN, aber ohne Wortlaut oder ohne Naehrwerte."},
